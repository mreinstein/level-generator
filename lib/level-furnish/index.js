...
export default function furnish (machines, objects, theme, option={}) {
  ...
  for (let entity of Array.from(objects)) {
    if ((entity.type === 'room') || (entity.type === 'anteroom') || (entity.type === 'tunnel')) {
      ...
      if ((composition === 'machines') || (composition === 'machines_stockpile')) {
        const machine = selectMachine(machines, theme, entity.type)

        const result = layout(entity, machine)
        for (let m of Array.from(result))
          allMachines.push(m)
      }
    }
  }
...
function layout (entity, machine, rotate) {
  ...
  while (y < (entity.y + entity.height)) {
    ...
    if (y < (entity.y + entity.height)) {
      ...
      for (let m of Array.from(nextRow))
        m.y = y
    }
    y += height
...
function selectMachine (machines, theme, entityType) {
